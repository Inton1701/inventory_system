      ...
      <div class="page-header">
        <div class="add-item d-flex">
          <div class="page-title">
            <h4>Voided Transaction List</h4>
            <h6>Manage Your Voided Transaction</h6>
          </div>
        </div>
      </div>
                    ...
                    <ul class="dropdown-menu">
                      <li>
                        <a
                          ...
                          @click="showSaleDetail(trans.transactionId)"
                        >
                      ...
                      <li>
                        <a
                          href="javascript:void(0);"
                          class="dropdown-item"
                          @click="downloadReceipt(trans.transactionId)"
                        >
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <tr
                v-for="product in selectedTransaction.products"
                :key="product._id"
              >
                <td>{{ product.name }}</td>
                <td>{{ product.sku }}</td>
...
export default {
  ...
  setup() {
    ...
    const getTransactions = async () => {
      loading.value = true;
      try {
        const reponse = await axios.get(`${apiURL}/get_voided_transactions`);
    ...
    const getTransactionDetails = async (transactionId) => {
      ...
      try {
        const response = await axios.get(
          `${apiURL}/get_transaction_details/${transactionId}`
        );
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.patch(
            `${apiURL}/delete_transaction/${transactionId}`
          );
          if (response.data.success) {
            Swal.fire(
              "Deleted!",
    ...
    const downloadReceipt = async (transactionId) => {
      try {
        const response = await axios.get(
          `${apiURL}/download_receipt/${transactionId}`,
          {
